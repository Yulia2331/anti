jQuery( document ).ready( function () {
	...
	function set_like( $button ) {
		let element = document.getElementById("kt_header_user_menu_toggle");
		
		if(element){
			var client_id = document.getElementById("kt_header_user_menu_toggle").getAttribute('data-user-id');
		}
	  ...
	  jQuery.ajax( {
		 ...
		 success: function ( answer ) {
			...
			$button.find('.count').text(answer.data.count);
		 },
	...
	if ( client_id == null ) {
	  client_id = new String( navigator.userAgent + ' ' + date.toString() + ' ' ).hashCode();
