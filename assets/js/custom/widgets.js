...
var KTWidgets = function () {
    ...
    var initChartsWidget3 = function() {
        ...
        if (document.getElementById("kt_charts_widget_3_chart_pkm")){
            element = document.getElementById("kt_charts_widget_3_chart_pkm");
        }
        var height = parseInt(KTUtil.css(element, 'height'));
        var labelColor = KTUtil.getCssVariableValue('--bs-gray-500');
        var borderColor = KTUtil.getCssVariableValue('--bs-gray-200');
        ...
        let result = []
        if (document.getElementById("kt_charts_widget_3_chart_pkm")){
            result = get_zar_rezults_pkm();
        }
        else{
            result = get_zar_rezults();
        }
        ...
        var options = {
            series: [{
                name: 'Запланировано',
                data: Object.values(result.plan)
            },
            {
                name: 'Результат',
                data: Object.values(result.zar)
            ...
            xaxis: {
                categories: Object.keys(result.zar),
                axisBorder: {
                    show: false,
                },
                axisTicks: {
    ...
    var initUserFollowButton = function() {
        ...
        if (follow) {
            follow.addEventListener('click', function(e){
                ...
                // Check button state
                let queried = $(this).attr('data-queried');
                if (follow.classList.contains("btn-success")) {
                    $.ajax({
                        type:'POST',
                        url: "/wp-admin/admin-ajax.php",
                        data:{
                            action: 'user_desubscribe',
                            queried,
